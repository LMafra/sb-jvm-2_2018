...
void instance_frame_loader_interpreter(int indexname, int indextype, int indexclass, Instance * inst, cat1 * args){
	...
	Dprintf("[instance_frame_loader_interpreter]debug fl1\n");
	Frame_type * topush = (Frame_type *)calloc(1, sizeof(topush));
	topush->inst=inst;
	...
	#ifdef DEBUG
	printf("[instance_frame_loader_interpreter]debug fl2\n");
	printf("%d\n", methodname->length);
	printf("%c\n", methodname->bytes[0]);
	printf("%c\n", methodname->bytes[1]);
	printf("%c\n", methodname->bytes[2]);
	...
	DDprintf("[instance_frame_loader_interpreter]debug fl3\n");
	for(int i=0;i<methodtype->length;i++){
		mettype[i]=methodtype->bytes[i];
	}
	
	DDprintf("[instance_frame_loader_interpreter]debug fl4\n");
	...
	DDprintf("[instance_frame_loader_interpreter]debug fl5\n");
	/*metname[0]='p';
	metname[1]='e';
	metname[2]='n';
	metname[3]='\0';*/
	...
	if (condnotprintln(metname)==false){
		DDprintf("[instance_frame_loader_interpreter]debug fl10\n");
		double auxing;
		DDprintf("[instance_frame_loader_interpreter]debug fl11\n");
		((u4*)(&auxing))[0]=args[1].val;
		DDprintf("[instance_frame_loader_interpreter]debug fl12\n");
		((u4*)(&auxing))[1]=args[0].val;
		DDprintf("[instance_frame_loader_interpreter]debug fl13\n");
		Dprintf("%lf\n", auxing);
		DDprintf("[instance_frame_loader_interpreter]debug fl14\n");
		return;
	}

	
	DDprintf("[instance_frame_loader_interpreter]debug fl6\n");
	...
	Dprintf("[instance_frame_loader_interpreter]debug fl7 2\n");
	if (calledinfo->access_flags & enum_method_access_flags::ACC_STATIC){
		DDprintf("[instance_frame_loader_interpreter]debug fl static\n");
		for(int i=0;i<paramnum;i++){
			topush->variaveis_locais[i]=args[i];
		}
	}
	else{
		DDprintf("[instance_frame_loader_interpreter]debug fl no static\n");
		cat1 firstparam;
	...
	DDprintf("[instance_frame_loader_interpreter]debug fl9\n");
	return;
}

void instance_frame_loader(int index, Instance * inst, cat1 * args){
	
	DDprintf("[instance_frame_loader]debug fl1\n");
	Frame_type topush;
	topush.inst=inst;
	topush.PC_retorno=PC;
	...
	char classname[methodclassname->length+1];
	#ifdef DEBUG
	printf("[instance_frame_loader]debug fl2\n");
	printf("%d\n", methodname->length);
	printf("%c\n", methodname->bytes[0]);
	printf("%c\n", methodname->bytes[1]);
	printf("%c\n", methodname->bytes[2]);
	...
	DDprintf("[instance_frame_loader]debug fl3\n");
	for(int i=0;i<methodtype->length;i++){
		mettype[i]=methodtype->bytes[i];
	}
	
	DDprintf("[instance_frame_loader]debug fl4\n");
	for(int i=0;i<methodclassname->length;i++){
		classname[i]=methodclassname->bytes[i];
	}
	metname[methodname->length]='\0';
	...
	classname[methodclassname->length]='\0';
	if(condjavaclass(classname)&&condnotprintln(metname)){
		Dprintf("[instance_frame_loader]debug java not println\n");
		return;
	}
	
	DDprintf("[instance_frame_loader]debug fl5\n");
	/*metname[0]='p';
	metname[1]='e';
	metname[2]='n';
	metname[3]='\0';*/
	...
	if (condnotprintln(metname)==false){
		if(mettype[1]== 'D'){
			double auxing;
			DDprintf("[instance_frame_loader]debug fl11\n");
			((u4*)(&auxing))[0]=args[1].val;
			DDprintf("[instance_frame_loader]debug fl12\n");
			((u4*)(&auxing))[1]=args[0].val;
			DDprintf("[instance_frame_loader]debug fl13\n");
			printf("%lf\n", auxing);
			DDprintf("[instance_frame_loader]debug fl14\n");
			return;
		}
		if(mettype[1]== 'J'){
			int64_t auxing;
	...
	DDprintf("[instance_frame_loader]debug fl6\n");
	int paramnum = ammountofparams(mettype);

	method_info * calledinfo = findmethodinclass(inst,metname);
  Frame_type fr = frame_stack.top();
  frame_stack.pop();
  printf("pushed an poped ???\n");
	topush.variaveis_locais = (cat1*)malloc( (*(Code_attribute *)&calledinfo->attributes[0]).max_locals * sizeof(cat1) );

	
	DDprintf("[instance_frame_loader]debug fl7\n");
	
	if (calledinfo->access_flags & enum_method_access_flags::ACC_STATIC){
		DDprintf("[instance_frame_loader]debug fl static\n");
		for(int i=0;i<paramnum;i++){
			topush.variaveis_locais[i]=args[i];
		}
	}
	else{
		DDprintf("[instance_frame_loader]debug fl no static\n");
		cat1 firstparam;
		firstparam.ref_val=inst;
		topush.variaveis_locais[0] = firstparam;
		for(int i=0;i<paramnum;i++){
	...
	DDprintf("[instance_frame_loader]debug fl8\n");
	topush.PC_base = (*(Code_attribute *)&calledinfo->attributes[0]).code;
	PC = topush.PC_base;
	topush.PC_final = &topush.PC_base[(*(Code_attribute *)&calledinfo->attributes[0]).code_length];
	topush.initial_stack_size=jvm_stack.size();
	...
	DDprintf("[instance_frame_loader]debug fl9\n");
	return;
}